 ...
 */
class tx_femanagement_controller_qsm_antraege_meine extends tx_femanagement_controller_qsm_antraege {
	...
	function initSingleView() {		
		...
		$this->formView = t3lib_div::makeInstance($viewClassName,
																							...
																							'single_left',
																							$this->eidViewHandler);
		$this->formView->setControllerName('tx_femanagement_controller_qsm_antraege_meine');																		
	...
	function createAjaxData(&$view,&$data) {
		...
		if (!empty($data['args']['export'])) {
			$configArray['fields'] = 'uid,status,title,short_title,ziel,start,ende,bereich,einrichtung';
			$sqlQuery = $this->model->buildJoinQuery($configArray);
			$daten = $this->model->selectSqlData($sqlQuery);
			$fieldList = explode(',','uid,status,title,short_title,ziel,start,ende,bereichs_titel,einrichtungs_titel,antragsteller_name');
			$view->createDataExport($daten,$data['args']['export'],'QSM-Anträge',$fieldList);
			exit();
		} else {
			$configArray['fields'] = 'uid,status,title,start,ende,bereich,antragsteller,deleted,hidden';
			$sqlQuery = $this->model->buildJoinQuery($configArray);
			$out = '';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/controller/qsm/antraege/class.tx_femanagement_controller_qsm_antraege_meine.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/controller/qsm/antraege/class.tx_femanagement_controller_qsm_antraege_meine.php']);
}
