 ...
 */
class tx_femanagement_controller_calendar_organizer extends tx_femanagement_controller_calendar_main {
	...
	function initSingleView() {		
		$viewClassName = 'tx_femanagement_view_form_cal_organizer_single';
		$this->eidViewHandler =  $this->eidUrl . '&view=' . $viewClassName;
		$this->model = t3lib_div::makeInstance('tx_femanagement_model_cal_organizer',
																							 $this->piBase,
		...
		$this->formView = t3lib_div::makeInstance($viewClassName,
																							...
																							'cal_single',
																							$this->eidViewHandler);
		$this->formView->setControllerName('tx_femanagement_controller_calendar_organizer');																		
		$this->formView->setModelName('tx_femanagement_model_cal_organizer');																		
	}
	
	function initListView() {		
		$viewClassName = 'tx_femanagement_view_form_cal_organizer_list';
		$this->eidViewHandler =  $this->eidUrl . '&view=' . $viewClassName;
		$this->model = t3lib_div::makeInstance('tx_femanagement_model_cal_organizer',$this->piBase,$this->piBase->settings['STORAGE_PID']);
		$this->formView = t3lib_div::makeInstance($viewClassName,
																							$this->piBase,
																							$this->piBase->settings['STORAGE_PID'],
																							'Veranstalter',
	...
	function initFormSingle(&$formData,$mode='new') {		
		$fieldSettings['name'] = array(
												'title'=>'Bezeichnung des Veranstalters',
												'type'=>'input',
												'validate'=>'string',
												);
		if (parent::isAdmin()) {
	...
	function createFormSingle(&$formData,$mode='new') {		
		if (parent::isAdmin()) {
			$titelFelder = array('name','description',);										
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/controller/class.tx_femanagement_controller_calendar_organizer.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/controller/class.tx_femanagement_controller_calendar_organizer.php']);
}
