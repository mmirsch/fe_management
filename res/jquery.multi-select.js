...
(function($){
  var msMethods = {
    'init' : function(options){
      this.settings = {
        disabledClass : 'disabled',
				height: '200',
				width: '200',
  			selectableTitle: 'auswählbare Einträge',
  			selectedTitle: 'ausgewählte Einträge',
        emptyArray : false,
        callbackOnInit: false,
        keepOrder : false,
				mandatoryMsg: 'Dieser Eintrag kann nicht gelöscht werden!',
        mandatory : '',
				elem: ''
      };
      if(options) {
      ...
      multiSelects.each(function(){
				var listStyle = ' style="height: ' + multiSelects.settings.height + 'px; ' +
																	'width: ' + multiSelects.settings.width + 'px; ' +
																	'" ';
      	var listStyleContainer = ' style="height: ' + (parseInt(multiSelects.settings.height)+50) + 'px;" ';
      	var selectableTitle = '<div class="listTitle">' + multiSelects.settings.selectableTitle + '</div>';
				var selectedTitle = '<div class="listTitle">' + multiSelects.settings.selectedTitle + '</div>';
        var ms = $(this);
        if (ms.next('.ms-container').length == 0){
          ...
          var container = $('<div id="ms-'+ms.attr('id')+'" class="ms-container"></div>'),
              selectableContainer = $('<div class="ms-selectable"' + listStyleContainer + '>' + selectableTitle + '</div>'),
              arrowContainer = $('<div class="ms-arrow"></div>'),
              selectedContainer = $('<div class="ms-selection"' + listStyleContainer + '>' + selectedTitle + '</div>'),
              selectableUl = $('<ul ' + listStyle + ' class="ms-list"></ul>'),
              selectedUl = $('<ul ' + listStyle + ' class="ms-list"></ul>');
      ...
      var selectedLi = $('<li class="ms-elem-selected'+(klass ? ' '+klass : '')+'" ms-value="'+value+'">'+text+'</li>'),
          ...
          haveToSelect =  ((!selectableLi.hasClass(ms.data('settings').disabledClass)) || 
                          ...
                          ((method == 'init' && selectedOption.attr('selected')) ||
                            (method != 'init' && !selectedOption.attr('selected')));
			var errorElem = "#error_" + ms.data('settings').elem;
			$(errorElem).detach();
      ...
      if (haveToSelect ){
        ...
        if (selectableLi.hasClass(ms.data('settings').disabledClass)){
          ...
          selectedLi.click(function(){
            ms.multiSelect('deselect', $(this).attr('ms-value'),this);
          });
        }
    ...
    'deselect' : function(value,elem){
			var mandatory = this.data('settings').mandatory;
			for (var i = 0; i < mandatory.length; i++) {
		  	if (value == mandatory[i]) {
		  		msgBox(elem.offsetLeft, elem.offsetTop, this.data('settings').mandatoryMsg);
		  		return;
		  	}
		  }
	     var ms = this,
