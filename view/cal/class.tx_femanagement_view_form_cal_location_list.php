 ...
 */
class tx_femanagement_view_form_cal_location_list extends tx_femanagement_view_form_list {
	...
	function showListItem($elem,$fieldList,$permissions,$rowClass='') {
		...
		foreach ($fieldList as $key=>$field) {
			...
			case 'name':
				...
				break;
			case 'tx_femanagement_cal_room':
			case 'tx_femanagement_cal_building':
			case 'anz_termine':
				$out .= $elem[$key];
				break;	
			case 'tx_femanagement_cal_campus':
				$out .= $this->model->getCampusTitle($elem[$key]);
				break;
			}
	...
	function ajaxFilter($data) {
		...
		$configArray['joins'] = array(
				array('table'=>'tx_cal_event',
						'count'=>'location_id AS anz_termine',
						'joinFieldLocal'=>'location_id',
						'joinFieldMain'=>'uid',
						'mode'=>'LEFT JOIN',
						'groupBy'=>'tx_cal_location.uid',
				),
		);
		$configArray['all_pids'] = 'TRUE';		
		$configArray['sqlFilter'] = 'TRUE';
		$configArray['fields'] = 'uid,name,tx_femanagement_cal_campus,tx_femanagement_cal_room,tx_femanagement_cal_building,deleted,hidden';
		...
		if (isset($this->args['volltextsuche']) && $this->args['volltextsuche']!='') {
			$configArray['where'] .= ' AND (name LIKE "%' . $this->args['volltextsuche'] . '%" OR ' .
															 'tx_femanagement_cal_room LIKE "%' . $this->args['volltextsuche'] . '%")';
		}
		/*
		 * Sortierung prüfen
		*/
		if (!empty($this->args['sortField'])) {
			...
			switch ($this->args['sortField']) {
				...
				case 'tx_femanagement_cal_room':
				case 'tx_femanagement_cal_building':
					$sortField = 'tx_cal_location.' . $this->args['sortField'];
					break;
				case 'anz_termine':
