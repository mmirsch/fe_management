 ...
 */
class tx_femanagement_view_form_shop_lieferanten_list  extends tx_femanagement_view_form_list {
	...
	function emptyDataList() {
		return '<div class="info"><h3>Für Ihre Eingabe wurden keine Lieferanten gefunden!</h3>
	...
	function ajaxFilter($data) {
		...
		$configArray['where'] = ' WHERE tx_hebest_lieferanten.pid=' . $this->pid;
			...
			if (!empty($suchFelderListe)) {
				...
				foreach($suchFelderListe as $feld) {
					...
					switch ($feld) {
						case 'beschreibung':
						case 'title':
							$configArray['where'] .= ' OR tx_hebest_lieferanten.' . $feld . ' LIKE "%' . $this->args['volltextsuche'] . '%"';
							break;
						case 'stadt':
				...
				$configArray['where'] .= ' AND (tx_hebest_lieferanten.title LIKE "%' . $this->args['volltextsuche'] . '%")';
			}
		}
		if (isset($this->args['stadt'])) {
			if ($this->args['stadt']!='all') {
				$configArray['where'] .= ' AND (tx_hebest_lieferanten.stadt = (' . $this->args['stadt'] . '))';
			}
		}
	
		if (isset($this->args['keyword1'])) {
			if ($this->args['keyword1']!='all') {
				$configArray['where'] .= ' AND (tx_hebest_lieferanten.keyword1 = (' . $this->args['keyword1'] . '))';
			}
		}
	
		if (isset($this->args['keyword2'])) {
			if ($this->args['keyword2']!='all') {
				$configArray['where'] .= ' AND (tx_hebest_lieferanten.keyword2 = (' . $this->args['keyword2'] . '))';
			}
		}
	
		if (isset($this->args['hidden'])) {
			if ( $this->args['hidden']!='all') {
				$configArray['where'] .= ' AND tx_hebest_lieferanten.hidden=' . $this->args['hidden'];
			}
		}
		if (isset($this->args['deleted'])) {
			if ($this->args['deleted']!='all') {
				$configArray['where'] .= ' AND tx_hebest_lieferanten.deleted=' . $this->args['deleted'];
			}
		}
		if (isset($this->args['az']) AND $this->args['az']!='all') {
			$whereAz = ' AND (tx_hebest_lieferanten.title LIKE "' . $this->args['az'] . '%")';
		} else {
			$whereAz = '';
		}
		/*
		...
		*/
		if (!empty($this->args['sortField'])) {
			...
			switch ($this->args['sortField']) {
				case 'hauptkategorie':
					...
					break;
				default:
					$sortField = 'tx_hebest_lieferanten.' . $this->args['sortField'];
				break;
			}
			$configArray['orderBy'] = $sortField . ' ' . $sortMode;
		} else {
			$configArray['orderBy'] = 'tx_hebest_lieferanten.title ASC';
		}
		if (!empty($this->args['export'])) {
			...
			$this->createDataExport($daten,$this->args['export'],'lieferanten');
			exit();
		} else {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/view/shop/class.tx_femanagement_view_form_shop_lieferanten_list.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/fe_management/view/shop/class.tx_femanagement_view_form_shop_lieferanten_list.php']);
}
